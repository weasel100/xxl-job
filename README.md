...
##  自定义版本说明--这中一个ORACLE数据库版
-  与XXL-JOB 使用上基本没有修改，就是把帮助页面给删除了
-  为了支持ORACLE数据库，admin里针对数据库的操作，以及JAVABEAN都有很大的修改，不支持再从主分支上合并
-  任务执行器只我只需要springboot版，所以其它执行器都被我删除了，但是功能本身没有修改，这部分是可以升级的。
## Documentation
